...
export function validateShippingForm(data: {
  ...
  if (!isValidName(data.fullName)) {
    errors.push("Please enter a valid full name (letters only, at least 2 characters)")
  }
  ...
  if (!isValidPhone(data.phone)) {
    errors.push("Please enter a valid Sri Lankan phone number (e.g., 07X XXX XXXX)")
  }
...
/** Validate review */
export function validateReview(data: { rating: number; comment: string }): {
  valid: boolean
  errors: string[]
} {
  const errors: string[] = []
